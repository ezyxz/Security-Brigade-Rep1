...
type SearchResult1 []SearchResult

func (s SearchResult1) Len() int {
	return len(s)
}
func (s SearchResult1) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}
func (s SearchResult1) Less(i, j int) bool {
...
func (si *SearchIndex) Query(s []int) []SearchResult {
	...
	sort.Sort(sort.Reverse(SearchResult1(results)))
	return results
}
